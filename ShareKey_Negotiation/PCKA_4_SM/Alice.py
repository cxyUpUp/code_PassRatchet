...
#     """发送消息"""
#     r_ = gamma_.r
#     key_bytes = kdf_bytes(int_from_point(gamma_.k).to_bytes(64, 'big'))
#     # print("DDDDebug gamma_.kA", gamma_.k)
#     c = SE_enc(key_bytes, m)
#
#     # sample new r'
#     r_prime = secrets.randbelow(order - 1) + 1
#     hk_scalar = int_from_point(gamma_.k) % order
#     alpha_ = r_prime * gamma_.k  # ECC 点乘
...
def run(pwA: bytes, sock):
    ...
    # df.to_excel(r"D:\pyProiect\code_PassRatchaet\Experiment_result\share_pw_PCKA.xlsx", index=False)
    # df.to_excel(r"D:\pyProiect\code_PassRatchaet\Experiment_result\share_para_PCKA.xlsx", index=False)
    ...
    # with pd.ExcelWriter(r"D:\pyProiect\code_PassRatchaet\Experiment_result\pw_PCKA.xlsx",
    #                     engine='openpyxl',
    #                     mode='a') as writer:
    #     df.to_excel(writer, sheet_name='128-1', index=False)

    # with pd.ExcelWriter(r"D:\pyProiect\code_PassRatchaet\Experiment_result\para_PCKA.xlsx",
    #                     engine='openpyxl',
    #                     mode='a') as writer:
    #     df.to_excel(writer, sheet_name='256-1', index=False)

    # with pd.ExcelWriter(r"D:\pyProiect\code_PassRatchaet\Experiment_result\share_pw_PCKA.xlsx",
    #                     engine='openpyxl',
    #                     mode='a') as writer:
    #     df.to_excel(writer, sheet_name='share16-1', index=False)

    # with pd.ExcelWriter(r"D:\pyProiect\code_PassRatchaet\Experiment_result\share_para_PCKA.xlsx",
    #                     engine='openpyxl',
    #                     mode='a') as writer:
    #     df.to_excel(writer, sheet_name='share-192-', index=False)


    # with pd.ExcelWriter(r"D:\pyProiect\code_PassRatchaet\Experiment_result\PCKA_Snd.xlsx", engine='openpyxl',
    #                     mode='a') as writer:
    #     df1.to_excel(writer, sheet_name='Snd_256', index=False)
    #
    # with pd.ExcelWriter(r"D:\pyProiect\code_PassRatchaet\Experiment_result\PCKA_Rcv.xlsx", engine='openpyxl',
    #                     mode='a') as writer:
    #     df2.to_excel(writer, sheet_name='Rcv_256', index=False)
